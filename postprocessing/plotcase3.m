function [fig1, fig2, fig3] = plotcase3(case_in1,case_in2,case_in3,title)
...
lim1 = max([1-V2F1.DWX.T(end/2),V2F1.DWX.T(end/2),1-V2F1.RAD.T(end/2),V2F1.RAD.T(end/2)]);
lim3 = max([1-V2F3.DWX.T(end/2),V2F3.DWX.T(end/2),1-V2F3.RAD.T(end/2),V2F3.RAD.T(end/2)]);
lim2 = max([1-V2F2.DWX.T(end/2),V2F2.DWX.T(end/2),1-V2F2.RAD.T(end/2),V2F2.RAD.T(end/2)]);
...
h = legend({'standard model','radiative modification'},'location','northwest','interpreter','latex');
legend boxoff
p = get(h,'Position');
box on
...
plot(ydns(1:end/2),1-TD1(1:end/2),'o','color',color4,'MarkerSize',5,'Linewidth',1.0);
plot(2-ydns(end/2:end),TD1(end/2:end),'o','color',color3,'MarkerSize',5,'Linewidth',1.0);
plot(y(1:end/2),V2F2.RAD.THF(1:end/2)+100,'-','color',color3,'LineWidth',1.8);
ylabel('$| \overline{T} - T_w|$','Interpreter','latex');
axis([0 1 0 lim]);
set(gca,'xscale','log')
yticks([0 lim/3 lim/3*2 lim]);
