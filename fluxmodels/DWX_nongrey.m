...
function [ lam,t2,et,er,alphat ] = DWX_nongrey( T,Em,G,r,u,t2,et,k,e,alpha,mu,kP,kG,ReT,Pr,Pl,mesh,RadMod,kPMod,cP,cP2,Par1,Par2,Par3,used_kappa,Th,Tc,T0)

    n = size(T,1);
    
    tv       = (mesh.ddy*u).*mu;
    ut       = sqrt(tv(1)./2-tv(end)./r(end)/2);
    Retau    = ReT.*ut.*r;
    y        = mesh.y;
   	wallDist = min(y, 2-y);
    ...
    fg  = (fem.*(kC)+fkg.*(Em-G))./WVN.*atan(WVN./(kC));
    ...
    for i=2:n-1
    %    A(i,i) = A(i,i) - Cd2*fd2(i)*(et(i)+cr11.*er(i))/k(i).*r(i) - Cd1*fd1(i)*(et(i)+cr11.*er(i))/t2(i).*r(i);
    ...
    %b = - Cp1*sqrt(e(2:n-1).*(et(2:n-1))./k(2:n-1)./t2(2:n-1)).*Pt(2:n-1)./r(2:n-1);
    ...
    if(RadMod == 1)
        ...
        if(kPMod~=0)
            ...
            for i=2:n-1
                ...
                - 2*(Em(i)-G(i)).*fk(i);
                b(i-1) = b(i-1)...
                + (Em(i)-G(i)).*dfkdy(i).*dt2dy(i)/ReT/Pr... 
                + kP(i)*dCRdy(i).*dt2dy(i)/ReT/Pr...
                + fk(i).*dQdy(i).*dt2dy(i)/ReT/Pr...
                + (fem(i)-fg(i)).*dkPdy(i)*dt2dy(i)/ReT/Pr;
            end
        else
            for i=2:n-1
                A(i,i) = A(i,i) - 2*kP(i).* (fem(i)-fg(i)); 
